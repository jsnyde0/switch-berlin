...
Both shapes look benign at the write site and explode at the read site — sb-cm5 (2026-05-22) hit both in one session (events.0013 migration backfill + `Event.organizer` compat property). The clarification preserves D3's intent: data integrity is checked where the lie is *introduced*, not only where it surfaces.

...
| [ADR-012](ADR-012-event-visibility-tiers.md) | Event visibility tiers and access-control matrix | social-graph | Accepted 2026-05-21 (revised 2026-05-22) | `Event.visibility ∈ {public, semi_public, unlisted}` with source-derived `max(public)` defaults; viewer-tier × Event-tier access matrix configurable via `settings.EVENT_VISIBILITY_TRUSTED_STATUSES`; robot indexing derived from tier. |

...
| [ADR-010](ADR-010-event-based-product-posture.md) | Event-based product posture — facilitate real-world action, not platform engagement | product | Accepted 2026-05-20 (revised 2026-06-01 — Switch-as-canonical-home growth-loop application added) | Real-world action is the goal; engagement is an instrument, not an end; business model cannot monetize engagement. **Switch own-page is the canonical home for an event's full content; syndication to external platforms is downstream promotion — the growth loop (added 2026-06-01); UI anchors an event's canonical at the Switch channel tab.** |
...
| [ADR-012](ADR-012-event-visibility-tiers.md) | Event visibility tiers and access-control matrix | social-graph | Accepted 2026-05-21 (revised 2026-05-22 — D4 added; 2026-05-26 — read-side/syndication scope clarified) | `Event.visibility ∈ {public, semi_public, unlisted}` with source-derived `max(public)` defaults; viewer-tier × Event-tier access matrix configurable via `settings.EVENT_VISIBILITY_TRUSTED_STATUSES`; robot indexing derived from tier; D4 binds future migrations to de-facto-prior visibility as floor. **Scope clarified 2026-05-26: governs read-side rendering on switch.berlin ONLY — does NOT gate outbound syndication (ADR-016 D4/D5 owns that via explicit publish).** |
...
| [ADR-016](ADR-016-outbound-syndication-architecture-event-post-projections.md) | Outbound syndication architecture — canonical Event + Post, per-platform projections, agent + UI co-equal API clients | arch | Accepted 2026-05-25 (revised 2026-05-27; 2026-06-01 — ContentVersion generalized Event→publishable; 2026-06-05 — D4 post-hoc channel-add + D5 edit-after-publish dirty-marker + per-channel status) | Canonical Event and Post are separate entities (Event = structured facts, Post = communication artifact, one Event many Posts). Per-platform projections of kind ∈ {listing, promotion} point at a reusable `ContentVersion` via a `content_version` FK (2026-05-29 kb-wz8m evolution — replaces inline `override_data`; provenance moves onto the version; single-row sharing is the v0 default, divergence opt-in; **2026-06-01 — `ContentVersion` generalized from Event-only to a publishable (Event OR Post) via mutually-exclusive `event`/`post` FKs mirroring `PlatformProjection`; Posts get their own canonical seed so promo posts can diverge per-channel by ToS, e.g. Instagram vs Telegram; no formal `Publishable` base model**) + content-policy filtering (kb-o0j). D4: projections target a `PlatformConnection` destination (not a platform string), eager-fanned per enabled connection × supported kind. D5: explicit, actor-attested publish lifecycle (`mark-published` is a co-equal API verb; draft=WIP, draft→ready=completeness gate). Web UI and external agents are co-equal HTTP API clients (Moltbook-pattern Bearer + identity-token; v0 concrete shape pinned 2026-05-27 — one-time pairing-token redemption for issuance, key→identity-token exchange built, `verify-identity` stubbed until a third-party consumer exists, credential binds to User not Profile, actor-marker recorded for provenance only). D6: API framework = Django Ninja (in-process, shared service layer with HTMX views; django-bolt rejected — its separate Rust server with its own auth splits the single shared auth/service layer D3 requires). Visibility (ADR-012) is read-side-only and does NOT gate syndication — eager projection creation is uniform across all tiers; facilitator controls reach via explicit publish (D5). BYO-agent at v0; bundled-agent deferred. All decisions FLEXIBLE. |

| [ADR-017](ADR-017-authorization-edit-publish-policy.md) | Authorization policy — who may edit/publish an Event/Post/Projection | authz | Accepted 2026-05-26 (revised 2026-05-27) | Edit/publish authority = claimant of an `EventOrganizer` Profile (+ their paired agent); `EventFacilitator`s credited-only; rights derive from which through-table you're in, no new field. All authz routes through a single `can_edit` predicate (chokepoint, cheap foresight vs scattered checks). `ProfileClaim` is the team-membership seam — future roles/team-mgmt widen `ProfileClaim.role` + grant/revoke claims, no new model. D1 agent→claimant resolution settled 2026-05-27 (agent credential binds to the registering User; authority resolves through that User's full ProfileClaim set; mechanics in ADR-016 D3). All FLEXIBLE; authz distinct from ADR-016 D3 authn. |

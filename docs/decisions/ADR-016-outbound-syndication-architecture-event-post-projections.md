...
Each projection is generated initially from the canonical (Event for listing, Post for promotion) with content-policy filtering applied per platform (the filter rules live in kb-o0j cleaning-policy substrate, NOT in this ADR). Facilitator or agent edits the projection (per-field overrides) while it is in `draft`; flipping to `ready` freezes the effective content per the snapshot rule above. To propagate a later canonical edit into an already-`ready`/`published` projection, the facilitator or agent re-opens it (back to `draft` / re-generate), reviews, and re-publishes — an explicit act, never an automatic re-render of content that is already out.

...
def test_profile_claim_model_fields():
    ...
    user = User.objects.create_user(username="claimer_f", email="claimer_f@example.com", password="x")
...
def test_profile_claim_unique_together():
    ...
    user = User.objects.create_user(username="claimer_dup", email="claimer_dup@example.com", password="x")
...
def test_user_claimed_profiles_reverse_accessor():
    ...
    user = User.objects.create_user(username="claimer_rev", email="claimer_rev@example.com", password="x")
...
def test_profile_is_claimed_false_when_all_claims_revoked():
    ...
    user = User.objects.create_user(username="revoked_claimer", email="revoked_claimer@example.com", password="x")
...
def test_profile_is_claimed_true_when_one_active_among_revoked():
    ...
    user1 = User.objects.create_user(username="active_1", email="active_1@example.com", password="x")
    user2 = User.objects.create_user(username="revoked_2", email="revoked_2@example.com", password="x")
...
def test_profile_active_claimants_excludes_revoked():
    ...
    user1 = User.objects.create_user(username="active_ac", email="active_ac@example.com", password="x")
    user2 = User.objects.create_user(username="revoked_ac", email="revoked_ac@example.com", password="x")
...
def test_profile_active_claimants_empty_when_all_revoked():
    ...
    user = User.objects.create_user(username="all_revoked", email="all_revoked@example.com", password="x")
...
def test_profile_claim_inline_registered_in_admin():
    ...
    inline_model_classes = [inline.model for inline in model_admin.inlines if hasattr(inline, "model")]
    assert ProfileClaim in inline_model_classes, "ProfileClaim not found as inline on ProfileAdmin"
...
def test_profile_claim_inline_is_tabular():
    ...
    for inline in model_admin.inlines:
        if hasattr(inline, "model") and inline.model is ProfileClaim:
            assert issubclass(inline, TabularInline), "ProfileClaim inline should be TabularInline"
            return
...
def test_profile_claim_backfill_logic():
    ...
    user = User.objects.create_user(username="backfill_user", email="backfill@example.com", password="x")

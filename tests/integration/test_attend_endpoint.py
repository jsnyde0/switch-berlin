...
1. POST /events/<id>/attend/ with status=going updates or creates Attendance row.
...
4. Unauthenticated POST to /events/<id>/attend/ returns 302 to login page.
5. POST to non-existent event_id returns 404.
...
10. GET /events/<id>/attend/ returns 405 Method Not Allowed.
...
def test_attend_post_going_creates_row(client, staff_user, future_event):
    """POST /events/<id>/attend/ with status=going creates an Attendance row."""
    from events.models import Attendance

    client.force_login(staff_user)
    response = client.post(
        f"/events/{future_event.pk}/attend/", {"status": "going"}
    )
...
def test_attend_post_interested_creates_row(client, staff_user, future_event):
    """POST /events/<id>/attend/ with status=interested creates Attendance row."""
    from events.models import Attendance

    client.force_login(staff_user)
    response = client.post(
        f"/events/{future_event.pk}/attend/", {"status": "interested"}
    )
    assert response.status_code == 200
    assert Attendance.objects.filter(
...
def test_attend_post_updates_existing_row(client, staff_user, future_event):
    """POST /events/<id>/attend/ updates existing Attendance row (no duplicates)."""
    from events.models import Attendance
    ...
    client.force_login(staff_user)
    client.post(f"/events/{future_event.pk}/attend/", {"status": "going"})
...
def test_attend_response_has_hx_trigger_header(client, staff_user, future_event):
    """POST /events/<id>/attend/ response has HX-Trigger: events:attendance-changed."""
    client.force_login(staff_user)
    response = client.post(
        f"/events/{future_event.pk}/attend/", {"status": "going"}
    )
...
def test_attend_button_shows_went_for_past_event(client, staff_user, past_event):
    ...
    client.force_login(staff_user)
    response = client.post(
        f"/events/{past_event.pk}/attend/", {"status": "went"}
    )
    assert response.status_code == 200
    content = response.content.decode()
...
def test_attend_went_on_future_event_clamped_to_going(
    ...
    client.force_login(staff_user)
    client.post(f"/events/{future_event.pk}/attend/", {"status": "went"})
...
def test_attend_went_on_past_event_not_clamped(client, staff_user, past_event):
    ...
    client.force_login(staff_user)
    client.post(f"/events/{past_event.pk}/attend/", {"status": "went"})
...
def test_attend_unauthenticated_redirects_to_login(client, future_event):
    """Unauthenticated POST /events/<id>/attend/ returns 302 to login page."""
    response = client.post(
        f"/events/{future_event.pk}/attend/", {"status": "going"}
    )
...
@pytest.mark.django_db
def test_attend_nonexistent_event_returns_404(client, staff_user):
    """POST /events/99999/attend/ returns 404 for non-existent event."""
    client.force_login(staff_user)
...
def test_attend_unpublished_event_returns_404(client, staff_user, draft_event):
    ...
    client.force_login(staff_user)
    response = client.post(
        f"/events/{draft_event.pk}/attend/", {"status": "going"}
    )
    assert response.status_code == 404
...
def test_attend_get_method_not_allowed(client, staff_user, future_event):
    """GET /events/<id>/attend/ returns 405 Method Not Allowed."""
    client.force_login(staff_user)
    response = client.get(f"/events/{future_event.pk}/attend/")
...
def test_attend_idempotent_same_status(client, staff_user, future_event):
    ...
    client.force_login(staff_user)
    client.post(f"/events/{future_event.pk}/attend/", {"status": "going"})
    response = client.post(f"/events/{future_event.pk}/attend/", {"status": "going"})
...
def test_attend_invalid_status_clamped_to_interested(
    ...
    client.force_login(staff_user)
    response = client.post(
        f"/events/{future_event.pk}/attend/", {"status": "invalid_value"}
    )
    assert response.status_code == 200
    attendance = Attendance.objects.get(user=staff_user, event=future_event)

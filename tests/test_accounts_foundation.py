...
def test_auth_user_model_is_accounts_user():
    ...
    assert settings.AUTH_USER_MODEL == "accounts.User", f"Expected 'accounts.User', got '{settings.AUTH_USER_MODEL}'"
...
def test_accounts_app_in_installed_apps():
    ...
    assert "accounts" in settings.INSTALLED_APPS, f"'accounts' not in INSTALLED_APPS: {settings.INSTALLED_APPS}"
...
def test_locale_middleware_position():
    ...
    assert locale_idx == session_idx + 1, (
        f"LocaleMiddleware (idx {locale_idx}) must be immediately after SessionMiddleware (idx {session_idx})"
    )
...
def test_locale_paths_configured():
    ...
    assert "locale" in locale_path_names, f"No 'locale' directory in LOCALE_PATHS: {settings.LOCALE_PATHS}"
...
def test_user_model_is_custom():
    ...
    User = get_user_model()
    assert User.__module__ == "accounts.models", f"Expected accounts.models.User, got {User.__module__}.{User.__name__}"
...
def test_user_has_approved_at_field():
    ...
    assert user.approved_at is None, f"approved_at should default to None, got {user.approved_at!r}"
...
def test_user_has_approved_by_field():
    ...
    assert user.approved_by is None, f"approved_by should default to None, got {user.approved_by!r}"
...
def test_user_approved_by_self_fk():
    ...
    User = get_user_model()
    admin = User.objects.create_user(username="admin_user", email="admin@example.com", password="adminpass")
    regular = User.objects.create_user(username="regular_user", email="regular@example.com", password="regpass")
...
def test_vector_extension_migration_exists_in_a_core():
    ...
    assert a_core_migrations_dir.exists(), f"a_core/migrations/ directory not found at {a_core_migrations_dir}"

    migration_files = sorted(
        f.name for f in a_core_migrations_dir.iterdir() if f.name.endswith(".py") and f.name != "__init__.py"
    )
    vector_migrations = [f for f in migration_files if "vector_extension" in f]
    assert len(vector_migrations) == 1, (
        f"Expected exactly one vector_extension migration in a_core, got: {migration_files}"
    )
...
def test_user_migration_depends_on_vector_extension():
    ...
    migration_files = sorted(
        f for f in accounts_migrations_dir.iterdir() if f.name.endswith(".py") and f.name != "__init__.py"
    )
    file_names = [f.name for f in migration_files]
    assert len(migration_files) >= 1, f"Expected at least 1 migration file in accounts, got {file_names}"

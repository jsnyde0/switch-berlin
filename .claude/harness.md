...
- **SQLite for tests, Postgres in dev/prod.** `a_core/test_settings.py` substitutes SQLite `:memory:` and no-op migrations for `a_core` and `ingestion` to skip `pg_trgm` / `pgvector`. CI uses real pgvector/pg17 so migrations + Postgres extensions are exercised in CI even though local test runs skip them.

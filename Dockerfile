...
# Base: node:22-slim (sha256 pin per sb-mhi; Dependabot bumps the digest weekly).
...
# ---- Python app stage ----
# Base: python:3.13-slim-bookworm (sha256 pin per sb-mhi).
...
# Install uv from official image — pinned at 0.11.14 via sha256 (sb-mhi).
# Dependabot's docker ecosystem only watches FROM lines — COPY --from digests
# are not auto-bumped. Manual-bump cadence tracked in sb-q2z; refresh by hand
...
# collectstatic at build time. sb-x14 (2026-05-13) removed the SECRET_KEY default
...
# Non-root runtime (sb-a0c): create app user and chown /app so the init
